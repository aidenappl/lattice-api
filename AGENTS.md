...
| `install/runner.sh` | Embedded (`//go:embed`) worker install script served at `GET /install/runner` — and the script `upgrade_runner` runs on every worker (the runner verifies it against `expected_hash`, which `HandleWorkerActions` computes live from these bytes, so editing it needs no hash update). It writes the runner's systemd unit in two places (upgrade path + fresh install); both depend on Docker via `Wants=` + `After=` + `PartOf=docker.service`, **never `Requires=`** — one failed Docker start job leaves a `Requires=` unit "Dependency failed" and never retried, which stranded trailblaze-prod-worker-1 offline for three days in Sep 2026. The upgrade path also rewrites an existing `Requires=` unit in place. Pinned by `routers/install_script_test.go`; must match lattice-runner's `cmd/setup.go` `serviceTemplate`. |
